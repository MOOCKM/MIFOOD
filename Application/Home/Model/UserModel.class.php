...
class UserModel extends LoginModel {
    ...
    public function loves(){
        ...
        for($i=0;$i<$loves_num;$i++){
           $where['shop_id']=$loves[$i]['shop_id'];

           $shop[$i]=M('shop')->where($where)->select();
      }
        $_SESSION['loves_num']=$i;
    ...
    public function no_email(){
        $where['state']=0;
        $where['user_id']=$_SESSION['user_id'];
        $send=M('user')->where($where)->field('no_email')->select();
        if($send){
            return ture;
        }else{
            return false;
